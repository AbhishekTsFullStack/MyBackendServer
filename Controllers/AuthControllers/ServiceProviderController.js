...
const LoginServiceProvider = async (req, res) => {
  const { number, otp, otpid } = req.query; // Use req.body instead of req.params
  try {
    if (!number && !otp && !otpid)
      return res
        .status(400)
        .json({ error: true, message: "Invalid Credentials" });
    // check is Verified otp
    const isVarified = await isOptValid(otp, otpid);
    if (!isVarified)
      return res
        .status(400)
        .json({ error: true, message: "Otp Invalid or Expired" });
    // find the user
    const isUser = await ServiceProviderModel.findOne({ mobileNo: number });
    console.log(isUser)
    if (!isUser) {
...
const UpdateTheServiceProvider = async (req, res) => {
  ...
  try {
    const isUpdated = await ServiceProviderModel.findByIdAndUpdate(
      id,
      updatedData,
      { new: true }
    );
