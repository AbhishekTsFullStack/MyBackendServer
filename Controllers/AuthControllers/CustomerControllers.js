...
const SignupUser = async (req, res) => {
  try {
    ...
    const isUserFound = await CustomerModel.findOne({
      mobileNo: req.body.mobileNo,
    });
    if (isUserFound)
      return res
        .status(409)
        .json("You Are Already Our Customer Please Login In");
...
const LoginUser = async (req, res) => {
  ...
  try {
    // check the data is not equals to null
    if (!number && !otp)
      return res
        .status(400)
    ...
    // find the user
    const User = await CustomerModel.findOne({ mobileNo: number });
    if (!User)
...
const GetUpdateTheCustomer = async (req, res) => {
  ...
  try {
    const isUpdated = await CustomerModel.findByIdAndUpdate(
      id,
      {
        ...req.body,
      },
    ...
    res
      .status(200)
      .json({ error: false, data: isUpdated, message: "updated successfully" });
  } catch (error) {
    res.status(500).json({ error });
  }
};
