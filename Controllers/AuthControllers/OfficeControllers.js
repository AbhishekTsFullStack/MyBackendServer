...
const AddEmployee = async (req, res) => {
  ...
  try {
    ...
    if (!isReg)
      return res
        .status(400)
        .json({ error: true, message: "Not Register Please Try Again" });
...
//       isEmail(email) === true
//         ? "email"
//         : isMobileNumber(email) === true
//         ? "mobileNo"
//         : "Invalid Credential";
//     if (dataType === "Invalid Credential")
//       return res.status(400).json({
//         error: true,
//         message: "Please Enter the Valid Email Or Mobile Number",
//       });
...
//     if (isUser.role !== roles[`${logger}`])
//       return res
//         .status(404)
//         .json({ error: true, message: `Please Login In As per your Post ` });
//     console.log("Login as per your role");
...
const LoginEmployee = async (req, res) => {
  ...
  try {
    ...
    // find the uer
    const isUser = await EmployeeModel.findOne({ mobileNo: number });
    if (!isUser)
      return res.status(404).json({ error: true, message: "No user found" });
...
const UpdateTheEmployeeData = async (req, res) => {
  ...
  try {
    const isUpdated = await EmployeeModel.findByIdAndUpdate(id, updatedData, {
      new: true,
    });
    if (!isUpdated)
      return res
        .status(400)
        .json({ error: true, message: "Updation Failed Retry" });
...
const DeleteTheEmployeeData = async (req, res) => {
  ...
  try {
    ...
    if (!isDeleted)
      return res
        .status(400)
        .json({ error: true, message: "Not deleted try Again" });
