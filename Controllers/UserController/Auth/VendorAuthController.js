...
const VendorModel = require('../../../Models/UserModel/VendorModel')
...
const SignupUser = async (req, res) => {
    ...
    try {
        ...
        const isUserFound = await VendorModel.findOne({ mobileNo: req.body.mobileNo })
        if (isUserFound) return res.status(409).json("You Are Already Our Customer Please Login In")

        const isUserEmail = await VendorModel.findOne({ email: req.body.email });
        ...
        // my formdata
        const formdata = new VendorModel({
            ...req.body,
...
const LoginUser = async (req, res) => {
    ...
    try {
        ...
        // find the user 
        const User = await VendorModel.findOne(CheckField);
...
const AllCustomer = async (req, res) => {
    try {
        // check the user 
        const isUser = await VendorModel.find(req.body)
